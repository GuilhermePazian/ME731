...
packages= c("tidyverse","data.table","reshape2","captioner","gridExtra","xtable","MASS","xtable","knitr","kableExtra","quadprog","scales","ca","mice","Matrix")
...
dimnames(mental2) <- list(rownames,colnames)
names(dimnames(mental2)) <- c("Saúde Mental","Status Sócio-Econômico do País")
...
names(dimnames(perfil_linha)) <- c("Saúde Mental","Status Sócio-Econômico do País")


perfil_coluna = perfis_medios$m.C
dimnames(perfil_coluna) <- list(rownames,colnames)
names(dimnames(perfil_coluna)) <- c("Saúde Mental","Status Sócio-Econômico do País")
...
O banco de dados que vamos analisar é constituído de `r sum(mental3)` observações, com duas variáveis categorizadas que são: *Sáude Mental*, dividida em quatro categorias (Boa, Presença fraca de sintomas, Presença moderada de sintomas, Debilitado) e *Status Sócio-Ecõnomico do País*, dividido em cinco ordens (A(Boa),B,C,D e E(Baixo)). Os softwares utilizados foram: *R* \footnote{https://cran.r-project.org/}, versão 3.4.0 e *Rstudio* \footnote{https://www.rstudio.com/}, versão 1.0.1. 
...
```{r}
prop_total = round(mental2/sum(mental3)*100,2)
kable(round(mental2/sum(mental3)*100,2), format = "latex", booktabs = T)
```
...
O perfil das colunas é dado por:

```{r}

kable(perfil_coluna, format = "latex", booktabs = T)
```

Podemos ver que para quem possui o maior nível socio-economico temos que os perfis-medios relacionados aos estados de saúde mental Boa e o Debilitado  se destacam em relação a proporção na população com respectivamente `r round(perfil_linha[1,1],4)*100`%, `r round(perfil_linha[4,1],4)*100`%. Ou seja, há indícios a melhor condição sócio-econômica está relacionada com com a boa condição mental e ao estado debilitado.
...
```{r}
resultCA <- ca(mental2) # names(resultCA)
...
A proporção de variabilidade explicada é em torno de `r round(var_exp,4)*100` %. 
