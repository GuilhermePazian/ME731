...
b1 = dados %>% ggplot(aes(Reg, DGAD))+geom_boxplot()+theme_bw()
b2 = dados %>% ggplot(aes(Reg, DGM))+geom_boxplot()+theme_bw()
...
g1 <- dados %>% ggplot(aes(x = DGAD)) +
        geom_density(aes(fill=factor(Reg)),position="identity", alpha=0.5) +
...
```{r,fig.cap=c("Figura 1: Gráfico de dispersão entre os diâmetros da guelra de salmões em água doce e no mar"), fig.height=3}
...
```{r,fig.cap=c("Figura 2: Boxplots por grupo"), fig.pos = '!h', fig.height=3}
...
```{r,fig.cap=c("Figura 4: Quantil-quantil para cada Grupo"), fig.pos = '!h'}
library(car)
...
```{r, fig.height=3,fig.width=9,fig.cap=c(" Gráfico de quantil-quantil com envelopes para a distância de Mahalanobis; A) Alasca, B) Canadá"),fig.pos = '!h'}
library(car)
par(mfrow=c(1,2))
